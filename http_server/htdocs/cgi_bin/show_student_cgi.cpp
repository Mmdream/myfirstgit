...
int main()
{
	...
	std::cout<<"<head>show student`s information</head><br/>"<<std::endl;
...
void select_tab(char *const data_string)
{
	if(!data_string)
	{
		return;
	...
	sscanf(start_string,"%*s %s %*s %s",name,producer);
	...
	//strcat(post_data,",");
	strcat(post_data2,"\'");
	strcat(post_data2,producer);
	strcat(post_data2,"\'");
	//printf("%s",post_data);

	conn.select_mysql(post_data1,post_data2);
	std::cout<<"Select Success!<br/>"<<std::endl;
}
...
int main()
{
	...
	//conn.select_mysql(header,_sql_data,curr_row);
	std::cout<<"<table border=\"1\">"<<std::endl;
	std::cout<<"<tr>"<<std::endl;
